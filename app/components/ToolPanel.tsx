...
export function ToolPanel({ tools, selectedToolIds, onToggle, onSelectAll, onClearAll }: ToolPanelProps) {
  ...
  return (
    <div className="tool-panel" ref={panelRef}>
      <button
        className={`tool-panel-trigger${selectedToolIds.length > 0 ? ' is-active' : ''}`}
        type="button"
        onClick={() => setIsOpen((current) => !current)}
        aria-expanded={isOpen}
      >
        <span className="tool-panel-trigger-icon">
      ...
      {isOpen ? (
        <div className="tool-panel-popover glass-panel">
          <div className="tool-panel-header">
            ...
            <div className="tool-panel-subtitle">默认全部启用。发送消息时，只会绑定当前面板里勾选的工具。</div>
          </div>

          <div className="tool-panel-actions">
            <button className="tool-panel-action" type="button" onClick={onSelectAll}>
              全部启用
            </button>
            <button className="tool-panel-action" type="button" onClick={onClearAll}>
              全部关闭
            </button>
          </div>
          ...
          <div className="tool-panel-list">
            {tools.map((tool) => {
              ...
              return (
                <button
                  key={tool.id}
                  className={`tool-panel-item${selected ? ' is-selected' : ''}`}
                  type="button"
                ...
                >
                  <div className="tool-panel-item-row">
                    ...
                    {selected ? (
                      <span className="tool-panel-item-check">
